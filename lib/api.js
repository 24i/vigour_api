  ...
  define: {
    request (data) {
      ...
      req.on('response', function (message) {
        var [isOk, isRedirect, isBad, isError] = checkStatus(message)
        if (isError || isBad) {
          console.log('request error', 'isError', isError, 'isBad', isBad)
          api.emit('error', new Error(message.statusCode + ' raised by server'))
          return
        }
        if (isRedirect) {
